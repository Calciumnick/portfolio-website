...
const en = {
    ...
    projects: [
        ...
        { id: 2, title: "AutoFlow", category: "Business Automation · Private commission", technologies: "Python, Integrika CRM, PrivatBank API, WayForPay, Meta Ads, Dashboard", image: "", description: "A single management dashboard for a beauty-salon owner that replaces a morning of manual reporting with one screen. It pulls data live from six systems — Integrika CRM, PrivatBank, WayForPay, the Checkbox fiscal service, Meta Ads and Google Sheets — and turns it into real management metrics: revenue vs plan, net profit and cash flow, profitability per service line, KPIs for every master and admin, a CRM with LTV and churn, no-show losses and ad ROMI/ROAS. It forecasts month-end results and fires automatic alerts the moment something slips — plan under 80%, average check falling, no-show rising, stock or cash running low. The owner opens one page and sees the whole business — finance, team, clients, marketing — in two minutes. Built as a private commission — client-specific details and screens are kept under NDA.", link: "" },
...
const ru: typeof en = {
    ...
    projects: [
        ...
        { id: 2, title: "AutoFlow", category: "Бизнес-автоматизация · Частный заказ", technologies: "Python, Integrika CRM, ПриватБанк API, WayForPay, Meta Ads, Дашборд", image: "", description: "Единый управленческий дашборд для владельца салона красоты, который заменяет утро ручной отчётности одним экраном. Данные подтягиваются в реальном времени из шести систем — Integrika CRM, ПриватБанк, WayForPay, фискальный сервис Checkbox, Meta Ads и Google Sheets — и превращаются в реальные управленческие метрики: оборот против плана, чистая прибыль и cash flow, рентабельность по направлениям, KPI каждого мастера и администратора, CRM с LTV и оттоком, потери от no-show и ROMI/ROAS рекламы. Система прогнозирует итоги месяца и сама шлёт оповещения, как только что-то проседает — план ниже 80%, падает средний чек, растёт no-show, заканчиваются материалы или деньги. Владелец открывает одну страницу и за две минуты видит весь бизнес — финансы, команду, клиентов, маркетинг. Проект выполнен как частный заказ — детали и интерфейсы клиента остаются под NDA.", link: "" },
